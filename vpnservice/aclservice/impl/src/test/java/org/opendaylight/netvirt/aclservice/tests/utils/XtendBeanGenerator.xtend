...
class XtendBeanGenerator {
    ...
    def protected Class<?> getBuilderClass(Object bean) {
        if (bean.class.enclosingClass == null)
            bean.class
        else if (bean.class.enclosingClass.simpleName.endsWith("Builder"))
        ...
        for (propertyDescriptor : propertyDescriptors) {
            propertiesMap.put(propertyDescriptor.name,
                new Property(propertyDescriptor.name,
                        ...
                        // xtendReflectExtensions.invoke(bean, "get" + propertyDescriptor.name.toFirstUpper),
